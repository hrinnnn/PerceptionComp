...
"""
Bilibili视频下载和截取脚本
使用方法: python bilibili_downloader.py <config.json>
  ...
  "videos": [
    {
      ...
      "name": "video_name",
      "time_ranges": ["0m~5m", "5m~10m"]  // 可选，不填则下载整个视频
    }
  ]
}
"""
...
def download_video(url, output_path):
    """使用yt-dlp下载视频（分离音视频流后合并）"""
    print(f"正在下载视频: {url}")
    try:
        subprocess.run([
            'yt-dlp',
            '-f', 'bv[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/bv+ba/b',  # 分离下载最佳视频和音频
            '--merge-output-format', 'mp4',  # 合并为 mp4
            '--cookies-from-browser', 'chrome',  # 使用 Chrome 登录状态，绕过限制
...
def main():
    ...
    print("=" * 60)
    print("Bilibili 视频下载和截取工具 (从配置文件读取)")
    print("=" * 60)
    ...
    for video_idx, video_info in enumerate(videos, 1):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not time_ranges:
                ...
                print(f"\n开始保存完整视频:")
                print(f"[1/1] 完整视频")
                copy_video(temp_video, str(output_path))
            else:
                # 解析时间段
                try:
                    parsed_ranges = []
                ...
                for i, (start, end, range_str) in enumerate(parsed_ranges, 1):
                    # 生成输出文件名：如果只有一个段就是 name.mp4，多个段就是 name_1.mp4, name_2.mp4 等
                    if len(parsed_ranges) == 1:
                        output_filename = f"{video_name}.mp4"
                    else:
                        output_filename = f"{video_name}_{i}.mp4"
                    output_path = videos_dir / output_filename
