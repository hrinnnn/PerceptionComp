...
def run(
    ...
    for idx, item in enumerate(tqdm(data, desc="Classifying"), start=1):
        question = str(item.get("question", "")).strip()
        resume_id = make_resume_id(item.get("key"), question, idx)
...
def main():
    parser = argparse.ArgumentParser(
        description="Classify each question step into 6 visual abilities using Doubao API"
    )
    parser.add_argument(
        "--input_json",
        default="",
