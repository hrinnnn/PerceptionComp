...
def chat_with_multi_modal(
    ...
    # Configure generation with thinking budget if specified
    generation_config = {}
    ...
    if force_thinking:
        ...
        while (not has_think) and retries < max_retries:
            retries += 1
            follow_up = (
                "The previous response missed the required reasoning. "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", default="gemini-2.5-pro", type=str, help="Gemini model name"
    )
    parser.add_argument(
        "--api_key",
        default="AIzaSyBqA__5u13aTH8aTlyJmf3PG0yqjmKDusM",
        type=str,
        help="Google API key",
    )
    parser.add_argument(
        "--video_path",
        default="/Users/zhaozhixuan/Desktop/tsinghua learning/大二暑/暑研/tests/output_of_128_frames/processed_videos_128frames/",
        type=str,
    ...
    parser.add_argument(
        ...
        default=0,
        choices=[0, 1024, 4096],
        help="Thinking token budget: 0 (no limit), 1024, or 4096",
    )
